...
on_timer() {
  while(result_header.size()) {
    ...
    if(ipaddr1_filter!="") {
      if(result_ph.header_info.source != ipaddr1_filter.toStdString() &&
         result_ph.header_info.destination != ipaddr1_filter.toStdString()) {
        continue;
      }
    }
    if(ipaddr2_filter!="") {
      if(result_ph.header_info.source != ipaddr1_filter.toStdString() &&
         result_ph.header_info.destination != ipaddr2_filter.toStdString()) {
        continue;
      }
    }
    if(protocol_filter!="" &&
       result_ph.header_info.protocol != protocol_filter.toStdString()) {
      continue;
    }
...
{
  ipaddr1_filter = ui->ipEdit1->text();
  ipaddr2_filter = ui->ipEdit2->text();
  protocol_filter = ui->protocolEdit->text();
  ui->packetList->clear();
  ui->packetList->reset();
  for(int i=0;i<ph_list.size();i++) {
    if(ipaddr1_filter!="") {
      if(ph_list[i].header_info.source != ipaddr1_filter.toStdString() &&
         ph_list[i].header_info.destination != ipaddr1_filter.toStdString()) {
        continue;
      }
    }
    if(ipaddr2_filter!="") {
      if(ph_list[i].header_info.source != ipaddr1_filter.toStdString() &&
         ph_list[i].header_info.destination != ipaddr2_filter.toStdString()) {
        continue;
      }
    }
    if(protocol_filter!="" &&
